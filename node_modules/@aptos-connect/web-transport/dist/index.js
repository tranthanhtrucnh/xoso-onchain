...
var dismissalSerializedResponse = import_wallet_api.ConnectResponse.serialize({
  status: "dismissed"
});
  ...
  return new Promise((resolve, reject) => {
    const listeners = {
      onMessage: (message) => {
        ...
        if ((0, import_wallet_api.isTypedMessage)(import_wallet_api.PromptConnectionRequestMessage, message.data)) {
          promptWindow.postMessage(
            new import_wallet_api.PromptConnectionResponseMessage(request),
            baseUrl
          );
          return;
        }
...
async function createWalletRequest(baseUrl, request) {
  const encodedClientIdentityKey = (0, import_wallet_api3.bytesToBase64url)(
...
async function getWalletResponse(baseUrl, clientIdentityKey, requestId) {
  const encodedClientIdentityKey = (0, import_wallet_api4.bytesToBase64url)(
...
async function openTelegramPrompt(args) {
  ...
  const challenge = (0, import_wallet_api5.makePopupWalletRequestChallenge)({
    body,
    id: requestId,
    timestamp
  });
...
var WebWalletTransport = class {
  ...
  async sendRequest(request) {
    switch (request.name) {
      ...
      case import_wallet_api6.SignInRequest.name: {
        const serializedResponse = await this.sendPromptRequest(request);
        const response = import_wallet_api6.SignInResponse.deserialize(
          serializedResponse,
