...
function ed25519KeypairFromSecret(ed25519SecretKeyBytes) {
  return keypairToEd25519(
    import_tweetnacl.default.sign.keyPair.fromSeed(ed25519SecretKeyBytes.slice(0, 32))
  );
}
...
function encryptMessageDirect(senderX25519SecretKey, receiverX25519PublicKey, message) {
  ...
  const secured = import_tweetnacl2.default.box(
    messageUint8,
    nonce,
...
function messageHash(message, purpose) {
  const signaturePrefixHash = new Uint8Array(
    (0, import_sha3.sha3_256)(`${SIGNATURE_PREFIX}::${purpose}::`)
  );
  return new Uint8Array(
    (0, import_sha3.sha3_256)(concatUint8array(signaturePrefixHash, message))
  );
}
function signWithEd25519SecretKey(message, signingEd25519SecretKey, purpose) {
  return import_tweetnacl2.default.sign.detached(
...
function makeEd25519SecretKeySignCallbackNoDomainSeparation(signingEd25519SecretKey) {
  return async (message) => new import_ts_sdk.Ed25519Signature(
    import_tweetnacl2.default.sign.detached(message, signingEd25519SecretKey.key)
  );
}
...
function ensurePrivatePublicFieldsDisjoint(privateMessage, publicMessage) {
  const intersection = Object.keys(privateMessage).filter(
    (x) => Object.keys(publicMessage).includes(x)
  );
...
function verifyEnvelopeSignature(publicMessageBytes, privateMessageBytes, messageSignatureInput, senderEd25519PublicKey) {
  const messageSignature = new import_ts_sdk2.Ed25519Signature(
    import_ts_sdk2.Hex.fromHexInput(messageSignatureInput).toUint8Array()
  );
  ...
  if (ed25519SecretKeyOrSignCallback instanceof Function) {
    ...
    if (signature instanceof Promise) {
      return signature.then(
        (value) => ed25519KeypairFromSecret(value.toUint8Array())
      );
    }
