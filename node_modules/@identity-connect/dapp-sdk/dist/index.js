...
function normalizePayloadForIC(payload, aptosConfig) {
  ...
  return "multisigAddress" in payload ? {
    multisig_address: import_ts_sdk.AccountAddress.from(
...
var ACPairingClient = class {
  ...
  async createSigningRequest(pairing, type, networkName, requestBody) {
    ...
    const accountTransportEd25519PublicKey = (0, import_crypto.decodeBase64)(
  ...
  async signRequest(address, type, requestBody, { cancelToken, networkName } = {}) {
    ...
    const decrypted = (0, import_crypto.decryptEnvelope)(
      (0, import_crypto.toKey)(
        (0, import_crypto.decodeBase64)(pairing.accountTransportEd25519PublicKeyB64),
        import_crypto.KeyTypes.Ed25519PublicKey
      ),
      (0, import_crypto.toKey)(
        (0, import_crypto.decodeBase64)(pairing.dappEd25519SecretKeyB64),
        import_crypto.KeyTypes.Ed25519SecretKey
      ),
  ...
  async signMessage(address, args, options) {
    const response = await this.signRequest(address, import_api.SigningRequestTypes.SIGN_MESSAGE, args, options);
  ...
  async signTransaction(address, args, options) {
    ...
    const serializedResponseArgs = await this.signRequest(
      address,
      import_api.SigningRequestTypes.SIGN_TRANSACTION,
  ...
  async signAndSubmitTransaction(address, args, options) {
    ...
    try {
      const responseArgs = await this.signRequest(
        address,
        import_api.SigningRequestTypes.SIGN_AND_SUBMIT_TRANSACTION,
        serializedRequestArgs,
        options
      );
...
var ACDappClient = class {
  constructor({
    ...
    this.transport = new import_web_transport.WebWalletTransport(
      frontendBaseURL,
      provider,
      tgWebAppURL
    );
  ...
  async getKeylessAccounts() {
    const serializedRequest = import_wallet_api2.GetConnectedAccountsRequest.serialize(
      this.dappInfo
    );
  ...
  async isIcAccount(address) {
    ...
    return icAccounts.find(
      (account) => account.address.equals(import_ts_sdk3.AccountAddress.from(address))
    ) !== void 0;
  }
  async getConnectedAccounts() {
  ...
  async connect({
    ...
    const serializedRequest = import_wallet_api2.ConnectRequest.serialize(
      this.dappInfo,
      requestArgs
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
  ...
  async signIn(args) {
    const serializedRequest = import_wallet_api2.SignInRequest.serialize(this.dappInfo, args);
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
    const response = import_wallet_api2.SignInResponse.deserialize(serializedResponse);
    return response.args;
  }
  ...
  async signTransaction(args) {
    ...
    const serializedRequest = import_wallet_api2.SignTransactionRequest.serialize(
      this.dappInfo,
      normalizedArgs
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
  ...
  async signAndSubmitTransaction(args) {
    ...
    const serializedRequest = import_wallet_api2.SignAndSubmitTransactionRequest.serialize(
      this.dappInfo,
      args
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
...
var ACKeylessClient = class {
  ...
  async getConnectedAccounts() {
    const serializedRequest = import_wallet_api3.GetConnectedAccountsRequest.serialize(
      this.dappInfo
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
  ...
  async signTransaction(args) {
    ...
    const serializedRequest = import_wallet_api3.SignTransactionRequest.serialize(
      this.dappInfo,
      normalizedArgs
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
  ...
  async signAndSubmitTransaction(args) {
    const serializedRequest = import_wallet_api3.SignAndSubmitTransactionRequest.serialize(
      this.dappInfo,
      args
    );
    const serializedResponse = await this.transport.sendRequest(serializedRequest);
